...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/08/22 20:13:19 by mwubneh           #+#    #+#             */
/*   Updated: 2023/08/22 20:25:29 by mwubneh          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static t_arg	**get_files(t_arg **files, int i,
...
t_arg	**get_files_in_dir(char *path, t_garbage *garbage)
{
	...
	while (dir_entry != NULL)
	{
		if (ft_strcmp(dir_entry->d_name, ".") && \
			...
			files = get_files(files, i, dir_entry, garbage);
		dir_entry = readdir(current_dir);
	}
